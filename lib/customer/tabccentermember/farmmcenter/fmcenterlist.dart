...
import 'fmcenter.dart';
import 'fmcntdetailpage.dart';


class FmcHome extends StatelessWidget {


  @override
  Widget build(BuildContext context) {

    final fmcenterData = Provider.of<Fmcenters>(context);
    final fmcnts = fmcenterData.items;
    ...
    return Scaffold(
      ...
      itemBuilder: (context, i) => ChangeNotifierProvider.value(value: fmcnts[i],
      child: FmcenterList(
        name: fmcnts[i].name,
        place: fmcnts[i].place,
        imgUrl: fmcnts[i].imgUrl,
        growing: fmcnts[i].growing,
        phoneno: fmcnts[i].phoneno,
        //page: scbs[i].page,
        since: fmcnts[i].since,
        rating: fmcnts[i].rating,
        email: fmcnts[i].email,
        //price: trds[i].price,
      ),
      ),
    ),
...
class FmcenterList extends StatelessWidget {
  ...
  const FmcenterList({Key key,this.phoneno, this.landAcre, this.id, this.name, this.growing, this.since, this.place, this.rating, this.imgUrl, this.email}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final fmcnt = Provider.of<Fmcenter>(context);

    return GestureDetector(
      onTap: () {
        Navigator.of(context).pushNamed(FmcntDetailPage.routeName,arguments: fmcnt.place);
      },
      child: FittedBox(
      fit: BoxFit.contain,
      child: ClipRRect(

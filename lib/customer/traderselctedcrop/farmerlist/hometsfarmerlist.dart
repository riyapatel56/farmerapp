...
import 'detailtsfarmer.dart';
import 'tsfarmers.dart';

class TSFarmerHome extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final tsfarmerData = Provider.of<Tsfarmers>(context);
    final tsfs = tsfarmerData.items;
    ...
    return Scaffold(
      ...
      shrinkWrap: true,
      itemCount: tsfs.length,
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 1,childAspectRatio: 2.48),
      itemBuilder: (context, i) => ChangeNotifierProvider.value(value: tsfs[i],
      child: TsfarmerList(
        name: tsfs[i].name,
        place: tsfs[i].place,
        imgUrl: tsfs[i].imgUrl,
        growing: tsfs[i].growing,
        phoneno: tsfs[i].phoneno,
        //page: tsfs[i].page,
        since: tsfs[i].since,
        rating: tsfs[i].rating,
        email: tsfs[i].email,
        //price: trds[i].price,
      ),
      ),
...
class TsfarmerList extends StatelessWidget {
  ...
  const TsfarmerList({Key key,this.phoneno, this.landAcre, this.id, this.name, this.growing, this.since, this.place, this.rating, this.imgUrl, this.email}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final tsf = Provider.of<Tsfarmer>(context);

    return GestureDetector(
      onTap: () {
         Navigator.of(context).pushNamed(TsfDetailPage.routeName,arguments: tsf.id);
      },
      child: Padding(

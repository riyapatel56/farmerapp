...
import 'package:farmer/farmerpart/bottomnavbar/memabcfpo/abccentmember/trjcenter/trjcdetailpage.dart';
...
class TrjcHome extends StatelessWidget {


  @override
  Widget build(BuildContext context) {

    final trjcenterData = Provider.of<Trjcenters>(context);
    final trjcs = trjcenterData.items;
    ...
    return Scaffold(
      ...
      shrinkWrap: true,
      itemCount: trjcs.length,
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 1,childAspectRatio: 2.50),
      itemBuilder: (context, i) => ChangeNotifierProvider.value(value: trjcs[i],
      child: TrjcItem(
        name: trjcs[i].name,
        place: trjcs[i].place,
        imgUrl: trjcs[i].imgUrl,
        trading: trjcs[i].trading,
        phoneno: trjcs[i].phoneno,
        //page: scbs[i].page,
        since: trjcs[i].since,
        rating: trjcs[i].rating,
        email: trjcs[i].email,
        //price: trds[i].price,
      ),
      ),
...
class TrjcItem extends StatelessWidget {
  ...
  const TrjcItem({Key key,this.phoneno, this.landAcre, this.id, this.name, this.trading, this.since, this.place, this.rating, this.imgUrl, this.email}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final trjc = Provider.of<Trjcenter>(context);

    return GestureDetector(
      onTap: () {
        Navigator.of(context).pushNamed(TrjcDetailPage.routeName,arguments: trjc.place);
      },
      child: Padding(
